...
class EventFileHandler(logging.FileHandler):
    ...
    def __init__(self, filename, mode='a', encoding='utf-8', delay=False):
        """Create the file handler, forwarding all arguments to FileHandler.

        Args:
            filename: Path to the log file.
            mode: File open mode (default ``'a'`` to append).
            encoding: Text encoding for the file (default ``'utf-8'``).
            delay: If True, defer opening the file until the first write.
        """
...
def setup_logging():
    """Configure the root logger with console and date-based file handlers.

    Creates the logs directory, prunes old log files, attaches an INFO-level
    console handler and an :class:`EventFileHandler` writing to a
    ``YYYYMMDD.log`` file, and returns the configured root logger.
    ...
    # Tolerate characters the console encoding can't represent (e.g. cp1252 on
    # Windows) instead of raising a logging error on every unicode message.
    try:
...
class LoggerWriter:
    ...
    def __init__(self, logger, level):
        """Store the target logger and level, and init an empty buffer.

        Args:
            logger: Logger that received writes are forwarded to.
            level: Logging level (e.g. ``logging.INFO``) used for each write.
        """
        self.logger = logger
        self.level = level
        self.buffer = []
    ...
    def _stream(self):
        """Return the original stdout for INFO, otherwise the original stderr."""
        return sys.__stdout__ if self.level == logging.INFO else sys.__stderr__

    def write(self, message):
        """Log the message and mirror it to the original console stream.

        Non-blank messages are logged at the configured level; the text is also
        echoed to the original stdout/stderr. This is cp1252/None-safe: it does
        nothing when no console exists (``__stdout__``/``__stderr__`` is None) and
        falls back to a replaced-character encoding rather than raising on
        characters the console encoding cannot represent.

        Args:
            message: The text written to the stream.
        """
        ...
        # Also mirror to the original stdout/stderr for console visibility.
        # Guard against (a) a missing console — GUI / pythonw launches leave
        # __stdout__/__stderr__ as None — and (b) characters the console
        # encoding (e.g. cp1252 on Windows) can't represent, which would
        # otherwise raise UnicodeEncodeError and crash the writer.
        stream = self._stream()
        if stream is None:
            return
        try:
            ...
            stream.write(message.encode(enc, "replace").decode(enc, "replace"))
        except Exception:
            pass

    def flush(self):
        """Flush the original console stream if one exists.

        None-safe: does nothing when there is no console, and swallows any
        error raised while flushing.
        """
        # Flush the original stream if there is one
        stream = self._stream()
...
class TeeLogger:
    ...
    def __init__(self, logger, original_stream, log_level=logging.INFO):
        """Store the logger, the wrapped stream, and the log level.

        Args:
            logger: Logger that mirrored output is written to.
            original_stream: The original stream (console) to also write to.
            log_level: Logging level used for mirrored messages.
        """
        self.logger = logger
    ...
    def write(self, message):
        """Write the message to the console and log it with a ``CONSOLE:`` prefix.
    ...
    def flush(self):
        """Flush the wrapped console stream."""
...
def log_function_calls(func):
    ...
    def wrapper(*args, **kwargs):
        """Log entry, success, or failure around the wrapped call and return its result."""
        logger = app_logger_int
